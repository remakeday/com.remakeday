# REMAKE DAY — 작업지시서 v1.1 (Claude Code용)
...
**3개.** 상세와 역할별 정책 프롬프트는 `docs/spec/models_and_policies.md`(모델구성_정책프롬프트_v1)를 따른다. 이 절은 요약이다.

| 계층 | 역할 | 모델 | VRAM |
|---|---|---|---|
| 하위 | Agent (NPC 4인) | **exaone3.5:7.8b + 7세 정책** (기본) / gemma3:4b (게이트 비교) | 5.2 / 3.3GB |
| 중위·상위 | Planner, Advisor, Normalizer, Manager, Evaluator | **gemma3:12b** | 8.0GB |
| 임베딩 | 정답 주장·노트 검색 | **gemini-embedding** 온라인 (Qwen3-4B 로컬은 오프라인 폴백) | 0 |

- exaone + gemma3:12b 동시 상주 13.2GB — Life Tutorial 8/27 실측 구성. `OLLAMA_NUM_PARALLEL=1` 시작
- 중위를 따로 두지 않는다. 역할은 여섯, 모델은 셋
- Gemini flash-lite는 스위치 뒤 비교용. `fake`는 전 역할 필수
- gemma3:4b를 추가로 pull한다. 게이트에서 "정책 없이도 7세인가"를 본다 — 넘으면 그것이 하위 모델이고, 발표 문장 "모델 선택이 세계관"이 참이 된다. 못 넘으면 exaone + 정책으로 가고 문장을 "모델과 정책의 선택"으로 고친다

```env
NPC_LLM_PROVIDER=ollama   NPC_LLM_MODEL=exaone3.5:7.8b   NPC_AGE7_POLICY=on
CORE_LLM_PROVIDER=ollama  CORE_LLM_MODEL=gemma3:12b
EMBEDDING_PROVIDER=gemini EMBEDDING_FALLBACK=qwen3-local
SYSTEM_HARNESS=on  OLLAMA_NUM_PARALLEL=1  SCENARIO=a  COOKIE_AB=on  PAW_REASON_AB=on
```
