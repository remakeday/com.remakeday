...
**Goal:** 구글 OAuth 없이 `.env`의 개발 계정(id 001 / pw 001001)으로 `rd_session` 쿠키를 발급받아 허들(로그인 필수)을 통과할 수 있게 한다 — 2026-09-20 해커톤 제출 전까지 사용.
...
**Spec:** `docs/superpowers/specs/2026-09-16-abuse-guard-design.md` §1 1층(계정 게이트) + 사용자 지시(2026-09-16): 개발 계정 id 001 / pw 001001, `.env`로 관리, 9/20까지 유지. 검토 강도 **A**(인증).

## Global Constraints

- `.env` 키: `DEV_LOGIN=on`, `DEV_ACCOUNT_ID=001`, `DEV_ACCOUNT_PASSWORD=001001` (이미 `backend/.env`에 있음 — 수정 금지). 코드 기본값 `dev_login="off"` → 엔드포인트 404.
...
def test_endpoint_404_when_dev_login_off(monkeypatch):
    ...
    assert c.post("/api/v1/auth/dev/login", json={"id": "001", "password": "001001"}).status_code == 404
...
```bash
curl -s -c /tmp/dev.cookie -X POST http://127.0.0.1:8500/api/v1/auth/dev/login -H 'content-type: application/json' -d '{"id":"001","password":"001001"}'
```
