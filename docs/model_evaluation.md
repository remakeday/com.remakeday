...
**읽기.** 이 self-play는 F11(목표 점수대) 판단 재료가 못 된다 — 로컬 플레이어가 신의 질문을 한 번도 쓰지 않아(로그에 질문 0) 오늘 넣은 "물을수록 더 드러냄"(F5)이 전혀 작동하지 않았고, 4~5회차 0.0은 확인된 문장을 지우고 새로 쓴 결과(테스터10 F1과 같은 궤적, 플레이어가 `accepted_claims`를 읽지 않음). 사람 판(테스터12, F5 이전) 27.5와 비교 불가. 크래시·폴백·누설 축에서는 제출 구성에 이상이 없다.
