...
## 2026-09-19 — 밤 제출문 8칸 뭉침 해소 (사용자 실플레이 보고)
...
- **검증** — 백엔드 **1059 passed**, import 계약 4 kept, `tsc` 클린, 헤드리스 12종 중 11 통과(`dev-login`은 개발 로그인을 꺼 둔 상태라 의도된 실패). 서버 재기동 후 운영 설정 그대로(Sonnet 5/Haiku 4.5, 3판/200, 구글 로그인만), 새 경로는 비로그인에 401.
...
- **검증** — 백엔드 **1041 passed**(신규 `test_source_claims.py` 10건 포함), import 계약 4 kept, `tsc`, 헤드리스 11종 중 10 통과(`dev-login`은 개발 로그인을 끈 상태라 의도된 실패). 백엔드 재기동 후 설정 그대로(anthropic Sonnet 5/Haiku 4.5, 3판/200, 구글 로그인만) + `MAX_CLAIMS 40` 확인.
...
- **판 소유자 확인·공개 배포 강화**(`7141943`, 07:32) — 판 하위 17개 라우트에 로그인+판 주인 확인(`AttemptAccessInteractor`), 남의 판·없는 판·잘못된 ID를 같은 404로, 남의 `prior_attempt_id`는 없는 판처럼 무시. 공개 설정(https)에서는 인증 가드 off 기동 거부·Cloudflare 경유 요청 403, 인스펙터 토큰 기본값 제거·상수 시간 비교·헤더 전용, `/docs`·`/openapi.json` 비활성. 테스트 DB 이름이 `_test`가 아니면 거부.
...
### 오후 — 리서치 근거와 모델 평가 문장 대조·정정 (`00d4f86`, 16:24)

리서치 사이트 전수 검토에서 나온 지적을 정본에 되먹였다. `model_evaluation.md` 511줄(+298/−225)·`metrics.yml` 12줄.

- 맨 위에 **"최종 구성과 증거 범위" 표**를 두어 제출용(Sonnet 5·Haiku 4.5·gemini-embedding 2560)과 로컬 롤백(gemma4·kanana·bge-m3) 구성을 결정 위치·예외와 함께 한눈에 보이게 했다.
- **비열등은 역할별 게이트를 비교하는 운영 규칙**이지 통계 검정이 아니라고 명시. 공개 집계와 접근 제한 자료(`output/` 원문·프롬프트)를 구분해 적었다.
- 임베딩 `metrics.yml`의 `repeat_n3_identical` 필드를 뺐다 — n=3 반복은 초기 30문장에만 해당하고 검수 22문장은 단일 실행이라 필드가 사실을 과장하고 있었다. note에 그 구분을 적었다.
- 리서치 사이트 Exp 1 스냅샷은 이 커밋 뒤 18:28에 다시 떴다(A.20·A.21 반영, `01b115f`).
...
### 밤 — 배포 뒤 사용자 실플레이 검증 (23:32~00:03, 판 `8fc30a72`)

프로덕션 조합(Core Sonnet 5 · NPC Haiku 4.5)으로 사용자가 구글 계정으로 직접 플레이. **결론: 좋다(사용자).** 5회차 진행 중.

- **점수** — 1회차 35.0 → 2회차 39.4 → 3회차 73.8(통과) → 4회차 69.4(통과). 3·4회차 identity 100.
- **정체를 '돼지'라는 낱말 없이 맞혔다** — 인정된 문장은 "사람이 아니라 가축들인 것 같아."(identity-1·2 동시 인정). 채점 프롬프트의 동의어 규칙("동물"≈"가축"≈"기르는 것")이 의도대로 동작한 사례.
- **체감 지연** — 사용자: "채팅 올라오는 속도가 느려지고 장면 넘어갈 때 사진이 늦게 뜬다. 어쩔 수 없는 부분이고 신경쓰이는 정도는 아니다." 실측(이 판의 harness 기록, 호출당 평균/최대):

  | 모델 | 역할 | 호출 | 평균 | 최대 |
  |---|---|---:|---:|---:|
  | Sonnet 5 | planner(장면 조립) | 5 | 10.0s | 10.9s |
  | Sonnet 5 | advisor_answer | 4 | 4.7s | 5.7s |
  | Sonnet 5 | manager_check | 13 | 4.0s | 7.0s |
  | Sonnet 5 | evaluator_verdict | 40 | 3.6s | 5.3s |
  | Sonnet 5 | classifier | 27 | 2.3s | 7.0s |
  | Haiku 4.5 | agent(NPC 답변) | 28 | 2.0s | 3.1s |

  장면 전환이 느린 주범은 planner 10초. NPC 답변은 2초로 빠른 편. 남은 개선 수단은 Sonnet 프롬프트 캐시뿐이고 이는 코드 변경이라 제출 선언 뒤에는 불가 — 사용자에게 알림.
- **긍정 평가(사용자)** — 화면 문제 없음. 긴 정리형 질문에 그에 맞는 답변이 돌아온다. 중간 힌트가 늘었고 하루 마무리 힌트가 크게 도움. "단서" 표시가 좋다. 규칙을 원하는 방향으로 정할 수 있다.
...
- **`main` 배포(23:4x)** — 사용자 지시로 `main` ff 푸시(`b5441e1 → 7e7411a`) → Vercel success. 전환 뒤 재검증: 백엔드 1031 passed, tsc, 헤드리스 10/11(`dev-login`은 개발 로그인을 꺼서 의도된 실패), 공개 API·사이트 점검 기대값.
...
- **같은 코드에서 로컬 조합과 나란히(A.23 §5, 20:17)** — 로컬 gemma4/kanana vs Sonnet 5/Haiku 4.5. 제출 조합의 이점은 하나에 집중된다: **오답형 무득점(0 vs 14.6)** — gemma는 틀린 문장의 동기 칸에 점수를 준다. 정답·절반형은 칸별로 동일(채점기가 아니라 골든·매칭 규칙이 정한 값). 누설은 하네스 on이면 둘 다 0, off면 Haiku가 더 흘린다(50% vs 30%, 제출은 항상 on). self-play는 온라인이 더 빠르다(274s vs 306s, 로컬은 3모델 동주).
- **판당 비용 실측(A.23 §6, 20:25)** — self-play 판의 하네스 이벤트 92건 프롬프트·출력을 `count_tokens`로 세고 정가를 곱해 **기록분 $0.46**(밤 채점 0.17·관리자 검사 0.14·계획 0.07·NPC 0.04). 하네스에 안 남는 NPC 발화 95건을 더해 테스터12 규모(발화 113·질문 6·밤 5)로 환산하면 **판당 약 $0.9~1.2**, 09-17 추정 ~$0.8과 같은 자릿수. 캐시 미사용(`cache_read` 0) — NPC 시스템 프롬프트 ~3k tok을 캐시하면 판당 약 $0.6까지 내려가지만 제출 후 과제이므로 안 한다.
- **원숭이손 ① 아침 억제 반영 뒤 2차 실측(A.23 §7, 20:42~20:55, `155a15b`)** — 조합별로 서버를 재기동해 세 축을 다시 쟀다. 그림은 1차와 같다(오답 무득점·누설 on 0·온라인이 빠름). 절반형만 Sonnet이 원인 칸 부분 인정을 한 번 더 줘 54.8→59.2(n=1 변동). 아침 억제는 채점·누설 축과 무관한 시나리오 데이터라 그 축에 변화가 없고, DB 관찰 이벤트에서 2회차 1비트가 "채연은 오늘 쟁반을 옆으로 밀지 않는다"로 남는 것을 확인(`e5c911f`로 근거를 DB 이벤트로 정정).

### 밤 — 팀원 프론트 마감 커밋과 하루 3판 기본값 (22:35, `bc4b672`)
...
- **하루 3판이 기본값으로** — `Settings.user_daily_attempts` 5 → 3. `.env` 전환값에만 있던 값을 코드 기본값으로 내렸다. 테스트는 `test_user_daily_limit_is_five` → `test_user_daily_limit_is_three`(3판 200 → 4번째 403 `daily_attempt_limit`), 그동안 붙어 있던 "IP 분당 버킷 5와 하루 5판이 같은 수라 6번째에서 충돌한다"는 주석은 한도가 3으로 내려가 더는 성립하지 않아 정리했다. `docs/spec/api_contract.md` 기본값 표기도 5 → 3.
...
### 밤늦게 — 심사 피드백 6건 접수, 리서치 사이트 첫 화면·Exp 2 재집계, 발표 NPC 문장 (23:23~00:10)

배포가 끝난 뒤 제출물 비평 6건을 받았다. 대응은 리서치 사이트(research.remakeday.com, 별도 저장소, 미커밋)와 이 저장소의 문서 1건이다. 코드 변경은 없다.

- **리서치 첫 화면 순서** — 홈이 원칙 문구·NOT RUN 나열로 시작하고 게임 링크가 없었다. 맨 위 버튼을 "게임 먼저 해보기 → remakeday.com"으로, S1 그림 아래에 "지금까지 확정한 것" 5줄(372 실행 폴백 0·극성 게이트·라이선스 교체·14판 중 10판 완주·제출 조합)과 "우리가 아는 약점" 3줄(채점 안정성 미입증·재미 미측정·제출 구성 재측정 부족)을 Abstract 앞에 뒀다. 전부 기존 공개 문장의 재배치이고 각 줄이 근거 절로 링크된다. Abstract의 미측정 나열은 "미측정 항목" 절로 내렸다. S1 그림 라벨 01~04가 그림 하단에 붙어 잘려 보이던 여백(-3px → 18px)도 고쳤다.
- **Exp 2가 9/16에서 멈춰 있었다** — 사람 관찰 페이지는 스냅샷 스크립트가 없어 손으로 갱신하는데, 9/15 집계(6판·20건)와 9/16 테스터6 사례 뒤로 아무도 안 올렸다. 테스터7~12의 `review-verification/` 판 ID를 DB 읽기 전용 조회로 대조해 **사람 판 14판**으로 재집계했다: 규칙 60건 중 AI 계열 48(원숭이손 수락 17·추천 31)·직접 작성 12, 원숭이손 수락 17/21, 5회차 완주 10/14. 세션별 14행 표(판·루프·출처·점수 궤적)를 넣고 기존 6판 집계는 병기했다. 테스터7은 "미포함"에서 9/17 완주 판 `61acc8ef`로 정정. 새로 드러난 것: 4→5회차 점수 하락은 단조 잠금 도입(9/14) 전 4판에만 있고 이후 6판은 0건 — 잠금 작동의 증거이지 채점기 안정성 증거가 아니라고 적었다. Limitations에 채점 안정성·재미 미측정 항목 추가. 검증: check_public 0건 · refresh_research_data --check · Jekyll 빌드 · check_site --anchor-baseline(옛 Tester 7 앵커 span으로 보존) · 헤드리스 렌더 1280/400px.
- **planner maxItems(피드백 6번)는 이미 닫혀 있었다** — `ff8fa05`(11:39) `cut_to_max_length` 절단 + 테스트 4건 통과, 서버 기동(23:06)이 그 뒤라 프로덕션에 반영돼 있다. 리뷰어는 A.19 시점 문장을 본 것. 코드 변경 없음. 사이트 약점 3번째 줄을 "절단으로 닫았지만 절단 뒤 외부 API 재측정 없음"으로 고쳤다.
- **발표 스크립트 NPC 정책 문장(피드백 4번)** — 스크립트 파일이 저장소에 없어 교체 문장만 `docs/발표_NPC정책_문장_갱신_2026-09-18.md`에 뒀다. 근거 대조 결과 말투(7세)와 규칙의 문자 그대로 실행은 유지, "왜=몰라"·3턴 망각·무조건 전언 수용은 통과 조건에서 빠졌고 당일 기억·출처 구분이 들어왔다. NPC·planner에는 규칙 문장만 넘어가고 이유(`shown_reason`)는 플레이어 화면에만 남는다는 것을 코드로 확인해, 부작용 근거를 "해석자가 일곱 살이라서"에서 "규칙에 의도가 실리지 않아서 + 원숭이손 숨은 조건"으로 바꿨다. 구→신 문장 6쌍, 발표용 한 문단, Q&A 4줄.
- **재기동 보류** — 23:32부터 구글 계정 실사용자 1명이 3회차 진행 중(판 `8fc30a72`)이라 백엔드를 건드리지 않았다. 재미 게이트·방어선 ⑤는 사용자 지시로 다루지 않는다.
