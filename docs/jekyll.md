...
- **서버 실경로·VRAM 동주 — 여기서 결정이 갈렸다.** 8600/pigfarm_test에서 HTTP `/rule/preview` 임베딩 순위 확인("스피커실"→"방송실에 간다" 어간 3위→임베딩 1위), ollama 차단 시 200+어간 폴백. VRAM(RTX 5060 Ti 16GB): gemma4 8.1 + kanana 5.3 + **qwen3-embedding:4b 4.4GB(실상주) = 17.7GB → 임베딩 호출이 Core·NPC 둘 다 축출**(2/2, 재로드 4~6.5s). **bge-m3 0.66GB → 14,187 MiB 셋 다 온칩·축출 0**(2/2). 서버→ollama 본문 프록시 캡처로 지시문 미부착·1024 반환 실증.
