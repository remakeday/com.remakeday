...
허들이 켜지면 구글 없이 들어갈 길이 없어 개발 계정 로그인(id 001, `.env`의 `DEV_LOGIN`)을 A등급으로 시작했다. 백엔드 엔드포인트·유스케이스·테스트 11개까지 WIP(`7af14dd`, 585 passed)에서 사용자 지시로 중단. 남은 순서(검토·랜딩 폼·selfplay 플래그·머지·재기동)와 Anthropic 평가·F11·untrack 결정은 `HANDOFF-26-09-16.md` "지금 바로 할 일"에 있다.
