...
def ambient_probe(name, a_name, b_name, observation, llm):
    bundle = build().bundle()
    chars = {char.name: char for char in bundle.characters}
    a, b = chars[a_name], chars[b_name]
    prompt = prompts.AMBIENT_SYSTEM.format(
        a_name=a.name, b_name=b.name, a_rules='(없음)', b_rules='(없음)',
        a_persona='오늘 공개된 장면을 본다.', a_goal='본 것만 짧게 말한다', a_plan='지금 장면에 있다',
        b_persona='오늘 공개된 장면을 본다.', b_goal='들은 말과 본 일을 구분한다', b_plan='지금 장면에 있다',
        roster='채연, 민석, 은상, 준', absent_note='충식은 지금 여기 없다.',
    ) + '\n[현재 공개된 장면]\n' + observation + '\n연결되는 말이 없으면 빈 lines를 반환한다. 대사만으로 새로운 사건이 일어나지는 않는다.'

    def speakers(output):
        return next((f"speaker:{line.name}" for line in output.lines if line.name not in (a.name, b.name)), None)

    started = time.perf_counter()
    output, report = run_with_harness(
        llm, [system_msg(prompt)], AmbientOutput, role='ambient_probe', temperature=0.0,
        fact_checks=[speakers, unknown_person_check(['채연', '민석', '은상', '준'], fields=['text_all']),
                     korean_only_check(fields=['text_all']),
                     forbidden_word_check(['돼지', '돈사', '살처분', '열병'], fields=['text_all'])],
    )
    elapsed = round((time.perf_counter() - started) * 1000)
    return {
        'name': name, 'actors': [a_name, b_name], 'public_observation': observation,
        'output': output.model_dump(mode='json') if output else None,
        'elapsed_ms': elapsed, 'harness': asdict(report),
    }
...
def main():
    ...
    ambient = [
        ambient_probe('ration-rumor', '은상', '준', PUBLIC[6][4] + ' ' + PUBLIC[3][4], npc),
        ambient_probe('record-checkup', '민석', '채연', PUBLIC[4][4] + ' ' + PUBLIC[5][4], npc),
        ambient_probe('known-source', '은상', '민석', '은상이 말을 옮겼지만 출처는 직접 확인하지 못한 들은 말이라고 밝혔다.', npc),
    ]
    payload = {
        ...
        'questions': questions, 'custom_previews': previews, 'ambient': ambient,
        'review_template': {
