...
| Fake live API | Guarded `pigfarm_test`, fake providers, five loops, 25 checks, 117 request/response records, 0 failed checks. This run predates the final C3/RM repairs. | Preserved as integration evidence; not final-source smoke. |
...
Data policy: no production reset, migration, attempt creation or regrading during finalization. Tests use `pigfarm_test` under one owner. Existing records keep original scores and empty provenance where unmeasured. Automated executions are not human playtest samples.
