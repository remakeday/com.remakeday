...
(async () => {
  ...
  try {
    for (const [width, height] of [[390, 844], [1440, 900], [1564, 800]]) {
      ...
      await panel.getByRole('button', { name: '기록·안내 닫기', exact: true }).click();
      ...
      await page.getByText('복도 · 장면 2/6', { exact: true }).waitFor();
      await readAll(page);
