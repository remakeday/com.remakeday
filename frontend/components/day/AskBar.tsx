...
export function AskBar({ input, onInput, onAsk, onNext, inputLocked, nextLocked, waitingReply, waitingBeat, hint, placeholder, failure, onRetry, dayDone, onNight }: {
  ...
  return (
    <div className="shrink-0 border-t border-ink/15 px-3 py-2">
      ...
      {onRetry && <button type="button" onClick={onRetry} className="mb-2 rounded-ui border border-ink px-3 py-2 text-base">같은 질문 다시 보내기</button>}
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex min-w-0 basis-full items-center gap-2 sm:flex-1 sm:basis-auto">
          <input aria-label="인물에게 질문" aria-describedby="conversation-hint" type="text" maxLength={200}
            ...
            className="min-w-0 flex-1 rounded-ui border border-ink/40 bg-transparent px-3 py-2 text-lg outline-none placeholder:opacity-40 focus:border-ink disabled:opacity-40" />
          <button type="button" onClick={onAsk} disabled={inputLocked || !input.trim()} aria-busy={waitingReply}
            className="shrink-0 rounded-ui border border-ink px-3 py-2 text-lg hover:bg-ink hover:text-paper disabled:opacity-30">
            {waitingReply ? "답변 기다리는 중…" : "묻기"}
          </button>
        </div>
        <button type="button" onClick={dayDone ? onNight : onNext} disabled={nextLocked} aria-busy={waitingBeat}
          className="shrink-0 rounded-ui border border-ink/40 px-3 py-2 text-lg hover:border-ink disabled:opacity-30"
